...
if 0:
    ...
    class ArgParser():
        def __init__(self, data):
            ...
            self.dice_loss_weight    = data['dice_loss_weight']
...
print("seg_list 	     type: {} and value: {}".format(type(args.seg_list), args.seg_list))
...
print("labels            type: {} and value: {}".format(type(args.labels), args.labels))
...
print("atlas_seg         type: {} and value: {}".format(type(args.atlas_seg), args.atlas_seg))
...
print("dice_loss_weight  type: {} and value: {}".format(type(args.dice_loss_weight), args.dice_loss_weight))
...
train_segs = vxm.py.utils.read_file_list(args.seg_list, prefix=args.seg_prefix,
                                         suffix=args.seg_suffix)
assert len(train_imgs) > 0, 'Could not find any training data.'

print("len of train_imgs: {}".format(len(train_imgs)))
print("len of train_segs: {}".format(len(train_segs)))
assert len(train_imgs) ==  len(train_segs), 'Number of images and segmentation masks are not same.'
...
generator = vxm.generators.semisupervised(
    ...
    atlas_file_seg=args.atlas_seg, steps_per_epoch=args.steps_per_epoch)
